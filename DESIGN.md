...
So the pouch holds at most a dozen cards and the rest rotate. The character the kid was **just shown** is always present — newly met or re-shown for review, which are the same thing from the kid's point of view and must be treated as such. Nothing is ever dropped permanently, and which ones step aside is decided by the memory engine rather than at random: shaky characters keep their place, solid ones step aside, retired ones step aside first. The pouch becomes, quietly, a second scheduling surface.
